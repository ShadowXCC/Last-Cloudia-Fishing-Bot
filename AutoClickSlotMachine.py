...
if __name__ == '__main__':
    ...
    #            rect[1] + (.67 * (rect[3] - rect[1])))

    print("Taking over the mouse in:")
    print("3")
    time.sleep(1)
    print("2")
    time.sleep(1)
    ...
    while True:
        ...
        # mouse.move(rect[0] + (.5 * (rect[2] - rect[0])), rect[1] + (.67 * (rect[3] - rect[1])))
        # print("Moved mouse")
        # time.sleep(.5)
        ...
        # for i, pixel in enumerate(im.getdata()):
            # if pixel == (2,9,35): #IF SPECIFIC BLUE IS FOUND
                ...
                # time.sleep(7) # Longer sleep as animation takes a while to finish, even after clicking
